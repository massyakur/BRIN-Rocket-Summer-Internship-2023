...
for i = 2:length(t)
    ...
    if is_falling && altitude < max_throttle_height
        ...
        % change PD coeeficient if falling down
        Pp = 0.04;
        Dp = 0.0006;
    end
...
% graph for the elevator vs time
subplot(3, 2, 6);
plot(t, elevator_graph)
grid on;
ylim([-0.2, 1.2])
xlim([0, end_t])
xlabel('Time (s)');
